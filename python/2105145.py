...
def odd_even_decomposition(x : np.ndarray)->Tuple[np.ndarray, np.ndarray]:
    ...
    n=len(x)
    even_part=np.zeros(n)
    odd_part=np.zeros(n)
    # for i in range(len(x)):
    #     even_part[i]=(x[i]+x[-i-1])/2
    #     odd_part[i]=(x[i]-x[-i-1])/2
    # return odd_part,even_part
    even_part=0.5*(x[::1]+x[::-1])
    odd_part=0.5*(x[::1]-x[::-1])
    return odd_part,even_part
    None

def main():
    img_root_path = '/Users/sajidhasan/VSCODE/Web_Development'
    signal = init_signal()
    signal[INF] = 1
    signal[INF+1] = .5
    signal[INF-1] = 2
